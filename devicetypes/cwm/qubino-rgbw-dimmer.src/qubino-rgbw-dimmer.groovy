 ...
 *  Neil Cumpstey
 *
 *	Slightly amended from the version written by Kristjan Jam&scaron;ek (Kjamsek), Goap d.o.o.
 *  as it didn't work properly with my device.
 ...
 */
metadata {
  definition (name: "Qubino RGBW Dimmer", namespace: "cwm", author: "Neil Cumpstey", vid: "generic-rgbw-color-bulb") {
    capability "Actuator"
    capability "Switch"
  ...
  preferences {
    ...
    input name: "param1", type: "enum", required: false,
      options: [
        "1" : "1 – NORMAL mode – momentary switch type",
    ...
    input name: "param5", type: "enum", required: false,
      options: [
        "1" : "1 – Ocean (soft flowing between shades of blue color)",
    ...
    input name: "param7", type: "enum", required: false,
      options: [
        "0" : "0 – device does not memorize its status at power cut. Load is disconnected.",
...
/**
 * Event handler for received BasicReport commands.
 *
 * @param cmd communication frame
*/
def zwaveEvent(physicalgraph.zwave.commands.basicv1.BasicReport cmd){
  log.debug "Qubino RGBW Dimmer: firing BasicReport: ${cmd}"
